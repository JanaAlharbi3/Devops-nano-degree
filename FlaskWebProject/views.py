...
# def authorized():
#     logger.info("Authorization response received from Microsoft.")
...
#         logger.error(f"Error during authentication: {request.args}")
...
#         # Store Microsoft user claims in session
#         session["user"] = result.get("id_token_claims")
#         logger.info(f"Microsoft login successful for: {session['user'].get('name')}")

#         # Auto-login as the 'admin' user in your DB
...
#         )
#         if "error" in result:
#             logger.warning(f"Login failed: {result.get('error_description')}")
#             return render_template("auth_error.html", result=result)
#         session["user"] = result.get("id_token_claims")
#         logger.info(f"Admin logged in successfully: {session['user'].get('name')}")
#         user = User.query.filter_by(username="admin").first()
#         login_user(user)
#         _save_cache(cache)
#     return redirect(url_for('home'))
...
def authorized():
    ...
    if request.args.get('code'):
        ...
        if "error" in result:
            app.logger.warning(f"Microsoft login failed: {result.get('error_description')}")
            return render_template("auth_error.html", result=result)
        session["user"] = result.get("id_token_claims")
        app.logger.info(f"Microsoft login successful for: {session['user'].get('name')}")
        user = User.query.filter_by(username="admin").first()
        login_user(user)
        _save_cache(cache)
    return redirect(url_for('home'))
