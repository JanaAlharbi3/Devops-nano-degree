...
connect_str = f"DefaultEndpointsProtocol=https;AccountName={app.config['BLOB_ACCOUNT']};AccountKey={app.config['BLOB_STORAGE_KEY']};EndpointSuffix=core.windows.net"
...
class Post(db.Model):
    ...
    def save_changes(self, form, file, userId, new=False):
        ...
        if file:
            ...
            random_filename = id_generator() + '.' + file_extension

            try:
                # Upload new file
                file_stream = io.BytesIO(file.read())
                blob_container_client.upload_blob(name=random_filename, data=file_stream, overwrite=True)
